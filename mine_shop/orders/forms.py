...
class OrderInformationForm(forms.Form):
    name = forms.CharField(required=True, max_length=50, min_length=2, widget=forms.TextInput(attrs={'class': 'form-control;'}),
                           error_messages={"required": _("Обязательное условие"), }, label=_("Имя"))
    surname = forms.CharField(required=False, initial='', max_length=50, widget=forms.TextInput(attrs={'class': 'form-control;'}),
                           error_messages={'max_length': _("Не более 50 символов")}, label=_("Фамилия"))
    company_name= forms.CharField(required=False, initial='', max_length=100, min_length=3, widget=forms.TextInput(attrs={'class': 'form-control;'}),
    ...
    postcode = forms.IntegerField(required=False, initial=0, min_value=0, max_value=999999999999, widget=forms.TextInput(attrs={'class': 'form-control;'}),
                                   error_messages={'required': _("Обязательно условие"),
                                                                      'invalid': _("Неверное значение"),
                                                                     'max_value': _("Не более 12 символов"),
                                                                    'min_value': _("Не менее 2 символов")},
                           label=_("Почтовый код"))
    email = forms.EmailField(required=False, min_length=3, max_length=70, widget=forms.TextInput(attrs={'class': 'form-control;'}),
